...
public class TestAllCases {
  ...
  fun case21_3() {
    selectedIdx =
      if (!BuildConfig.IS_PRODUCTION) 0 else selectedIdx
    selectedIdx = selectedIdx.coerceAtLeast(0)
  }
  ...
  fun case21_5() {
    val x = if (!BuildConfig.IS_PRODUCTION) "local" else "intl"
    println(x)
  }
  ...
  fun case22_2() {
    beforeCode()
    if (!BuildConfig.IS_PRODUCTION) doSomething()
    afterCode()
  }
  ...
  fun case22_3() {
    beforeCode()
    if (BuildConfig.IS_PRODUCTION) doAction()
    afterCode()
  }
